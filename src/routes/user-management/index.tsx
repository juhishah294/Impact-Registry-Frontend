...
const UserManagement: React.FC = () => {
	...
	const [selectedTab, setSelectedTab] = useState<'institutes' | 'users'>(
		'institutes'
	);
	...
	} = useQuery(GET_PENDING_INSTITUTES);
	const {
		data: approvedData,
		loading: approvedLoading,
		refetch: refetchApproved,
	} = useQuery(GET_APPROVED_INSTITUTES);
	...
	const filteredInstitutes = useMemo(() => {
		...
		if (statusFilter !== 'all') {
			filtered = filtered.filter(
				(institute) => institute.approvalStatus === statusFilter
			);
		}
	...
	const filteredUsers = useMemo(() => {
		...
		if (instituteFilter !== 'all') {
			filtered = filtered.filter(
				(user) => user.instituteId === instituteFilter
			);
		}
	...
	const handleToggleInstituteStatus = async (
		instituteId: string,
			...
			<div className='flex items-center justify-between'>
				<div>
					...
					<p className='text-gray-600 mt-1'>
						Manage institutes and users across the system
					</p>
				</div>
				<div className='flex items-center space-x-4'>
					<div className='flex items-center space-x-2'>
						...
						<span className='text-sm font-medium'>
							{allInstitutes.length} Institutes
						</span>
					</div>
					<div className='flex items-center space-x-2'>
			...
			{selectedTab === 'institutes' && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Manage institute approvals and status
						</CardDescription>
					</CardHeader>
					<CardContent>
												...
												<div className='flex items-center space-x-2'>
													{institute.approvalStatus === 'PENDING_APPROVAL' && (
														<>
															<Button
																size='sm'
																onClick={() =>
																	handleApproveInstitute(institute.id)
																}
													...
													{institute.approvalStatus === 'APPROVED' && (
														<Button
															...
															variant='outline'
															onClick={() =>
																handleSuspendInstitute(institute.id)
															}
														>
										...
										>
											...
											<td className='p-4'>
												<Button
													...
													variant='outline'
													onClick={() =>
														handleToggleUserStatus(user.id, user.status || 0)
													}
												>
													{user.status === 1 ? (
														<>
